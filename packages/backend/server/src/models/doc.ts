...
export class DocModel extends BaseModel {
  ...
  async delete(workspaceId: string, docId: string) {
    ...
    const { count: snapshots } = await this.tx.snapshot.deleteMany(ident);
    const { count: updates } = await this.tx.update.deleteMany(ident);
    const { count: histories } =
      await this.tx.snapshotHistory.deleteMany(ident);
    this.logger.log(
